    ...
    >
      <nav className="container mx-auto px-4 py-2 max-w-6xl">
        ...
        <div className="hidden md:flex items-center justify-center space-x-2 overflow-x-auto">
          {sections.map((section) => (
            <Link 
              ...
              href={section.href}
              scroll={false}
              className="text-xs text-gray-600 hover:text-yellow-600 transition-colors duration-300 px-2 py-1 rounded-md hover:bg-yellow-50"
              onClick={closeMenu}
            >
              {section.name}
            </Link>
        ...
        <div className="md:hidden flex justify-between items-center">
          <div className="text-sm font-semibold text-yellow-600">Sections</div>
          <button 
            onClick={toggleMenu} 
            ...
            <div className="flex flex-col">
              {sections.map((section) => (
                <Link 
                  ...
                  href={section.href}
                  scroll={false}
                  className="text-sm text-gray-700 hover:text-yellow-600 transition-colors duration-300 px-4 py-3 border-b last:border-b-0 hover:bg-yellow-50"
                  onClick={closeMenu}
                >
                  {section.name}
                </Link>
